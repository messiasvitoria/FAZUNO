...
const STATUS_CONFIG = {
  ...
  "aceita":     { color: "#16A34A", bg: "#F0FDF4", border: "#BBF7D0", label: "ACEITA" },
  "andamento":  { color: "#0284C7", bg: "#F0F9FF", border: "#BAE6FD", label: "EM ANDAMENTO" },
  "concluida":  { color: "#16A34A", bg: "#F0FDF4", border: "#BBF7D0", label: "CONCLUÍDA" },
  "cancelada":  { color: "#DC2626", bg: "#FEF2F2", border: "#FECACA", label: "CANCELADA" },
  "recusada":   { color: "#667085", bg: "#ECEFF3", border: "#D1D5DB", label: "RECUSADA" },
...
const MOCK = {
  ...
  reviews: 12,
  timeline: [
    { status: "Solicitação recebida", data: "20/05/2024 às 10:15", desc: "O cliente enviou a solicitação.", done: true, active: false },
    { status: "Aceita por você",      data: "20/05/2024 às 11:02", desc: "Você aceitou a solicitação.",    done: true, active: false },
    { status: "A caminho",            data: "20/05/2024 às 13:50", desc: "Você está a caminho do local.",  done: true, active: false },
    { status: "Em andamento",         data: "20/05/2024 às 14:05", desc: "Serviço em execução.",           done: false, active: true },
    { status: "Concluída",            data: null,                  desc: "Aguardando conclusão.",          done: false, active: false },
...
export default function DetalhesModal({ onClose, solicitacao }) {
  ...
  const podeReagendar = status === "aceita" || status === "Aceita" || status === "andamento" || status === "Em Andamento";
  ...
  return (
    <>
      ...
      <div className="modal-overlay" onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'DM Sans', sans-serif" }}>
        <div className="modal-box" onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 980, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(0,0,0,0.18)" }}>
          ...
          <div style={{ padding: "24px 28px 16px", borderBottom: "1px solid #F3F4F6" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <p style={{ fontSize: "0.8rem", color: "#9CA3AF", margin: 0 }}>ID: {sol.id || "—"}</p>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14, background: "#F9FAFB", borderRadius: 10, padding: "10px 16px", border: "1px solid #F3F4F6" }}>
              ...
              <span style={{ fontSize: "0.78rem", color: "#6B7280" }}>Última atualização: {sol.ultimaAtualizacao || sol.data || "—"}</span>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 0 }}>
            ...
            <div style={{ padding: "24px 28px", borderRight: "1px solid #F3F4F6" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: "0.85rem", fontWeight: 600, color: "#111827", display: "flex", alignItems: "center", gap: 5 }}>
                        <i className="ti ti-briefcase" style={{ fontSize: 15, color: "#6B7280" }} /> {sol.servico || sol.title}
                      </div>
                    </div>
                    <div>
                      <div style={{ fontSize: "0.7rem", color: "#9CA3AF", marginBottom: 3 }}>Descrição</div>
                      <div style={{ fontSize: "0.82rem", color: "#374151", lineHeight: 1.5 }}>{sol.descricao || sol.description}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <div>
                        <div style={{ fontSize: "0.7rem", color: "#9CA3AF", marginBottom: 3, display: "flex", alignItems: "center", gap: 4 }}>
                          <i className="ti ti-calendar" style={{ fontSize: 13 }} /> Data
                        </div>
                        <div style={{ fontSize: "0.85rem", fontWeight: 600, color: "#111827" }}>{sol.dataAgendada || sol.date || "—"}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: "0.7rem", color: "#9CA3AF", marginBottom: 3, display: "flex", alignItems: "center", gap: 4 }}>
                          <i className="ti ti-clock" style={{ fontSize: 13 }} /> Horário
                        </div>
                        <div style={{ fontSize: "0.85rem", fontWeight: 600, color: "#111827" }}>{sol.horarioAgendado || sol.time || "—"}</div>
                      </div>
                    </div>
                    <div>
                      <div style={{ fontSize: "0.7rem", color: "#9CA3AF", marginBottom: 3, display: "flex", alignItems: "center", gap: 4 }}>
                      ...
                      <div style={{ fontSize: "1rem", fontWeight: 700, color: "#16A34A" }}>
                        {sol.valorEstimado || (sol.value ? `R$ ${sol.value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` : "—")}
                      </div>
                    </div>
                    {(sol.observacoes) && (
                      <div>
                        <div style={{ fontSize: "0.7rem", color: "#9CA3AF", marginBottom: 3 }}>Observações</div>
              ...
              <div style={{ background: "#fff", border: "1px solid #F3F4F6", borderRadius: 12, padding: "20px", marginBottom: 20 }}>
                <h3 style={{ fontFamily: "'Sora', sans-serif", fontSize: "0.92rem", fontWeight: 700, color: "#111827", margin: "0 0 16px", display: "flex", alignItems: "center", gap: 7 }}>
                  <i className="ti ti-user" style={{ fontSize: 17, color: "#6B7280" }} /> Informações do Cliente
                </h3>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    {clienteFoto ? (
                      <div style={{ width: 52, height: 52, borderRadius: "50%", overflow: "hidden", flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.12)" }}>
                        <img src={clienteFoto} alt={clienteNome} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>
                    ) : (
                      <div style={{ width: 52, height: 52, borderRadius: "50%", background: "#7C3AED", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: "1rem", color: "#fff" }}>
                        {clienteNome.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()}
                      </div>
                    )}
                    <div>
                      <div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#111827", marginBottom: 3 }}>{clienteNome}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <i className="ti ti-star" style={{ fontSize: 14, color: "#FBBF24" }} />
                        <span style={{ fontSize: "0.8rem", fontWeight: 600, color: "#111827" }}>{clienteRating}</span>
                        <span style={{ fontSize: "0.75rem", color: "#9CA3AF" }}>({clienteReviews} avaliações)</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="action-btn" onClick={() => router.push("/Pages/Perfil_cliente")} style={{ ...btnBase, background: "#fff", color: "#374151", border: "1.5px solid #E5E7EB", width: "auto", padding: "10px 18px" }}>
              ...
              <div>
                {(status === "nova") && (
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="action-btn" style={{ ...btnBase, background: "#16A34A", color: "#fff", border: "none" }}>
                      <i className="ti ti-check" style={{ fontSize: 15 }} /> Aceitar serviço
                    </button>
                ...
                {(status === "aceita" || status === "Aceita") && (
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="action-btn" style={{ ...btnBase, background: "#2563EB", color: "#fff", border: "none" }}>
                      <i className="ti ti-player-play" style={{ fontSize: 15 }} /> Iniciar serviço
                    </button>
                ...
                {(status === "andamento" || status === "Em Andamento") && (
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="action-btn" style={{ ...btnBase, background: "#16A34A", color: "#fff", border: "none" }}>
                      <i className="ti ti-circle-check" style={{ fontSize: 15 }} /> Concluir serviço
                    </button>
                    <button className="action-btn" onClick={() => setShowReagendamento(true)} style={{ ...btnBase, background: "#F5F3FF", color: "#7C3AED", border: "1.5px solid #DDD6FE" }}>
                      <i className="ti ti-calendar" style={{ fontSize: 15 }} /> Reagendar
                    </button>
                ...
                {(status === "concluida" || status === "Concluída") && (
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button className="action-btn" style={{ ...btnBase, background: "#EFF6FF", color: "#2563EB", border: "1.5px solid #BFDBFE" }}>
                      <i className="ti ti-file-invoice" style={{ fontSize: 15 }} /> Ver comprovante
                    </button>
                  </div>
                )}
                ...
                {(status === "recusada") && (
                  <button className="action-btn" style={{ ...btnBase, background: "#F3F4F6", color: "#6B7280", border: "1.5px solid #E5E7EB" }}>
                    <i className="ti ti-info-circle" style={{ fontSize: 15 }} /> Ver detalhes da recusa
                  </button>
                )}
              </div>
            </div>
                ...
                {timeline.map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: 12 }}>
                    ...
                    <div style={{ paddingBottom: 20, flex: 1 }}>
                      ...
                      {item.data && <div style={{ fontSize: "0.72rem", color: "#9CA3AF", marginBottom: 3 }}>{item.data}</div>}
